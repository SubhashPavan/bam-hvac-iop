        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="relative flex h-13 shrink-0 items-center gap-3 border-b border-navy-100 px-5 py-3 dark:border-slate-800/70">
            <div className="flex items-center gap-2.5">
              ...
              <div className="leading-tight"><div className="text-[13px] font-semibold tracking-tight">Inventory Optimization</div><div className="text-[8.5px] font-semibold uppercase tracking-[0.16em] text-accent-600 dark:text-accent-400">Admin · Platform</div></div>
            </div>
            <span className="mx-1 hidden h-5 w-px bg-navy-200 dark:bg-slate-700 sm:block" />
            <span className="hidden text-[13px] font-medium capitalize text-navy-500 dark:text-slate-400 sm:block">{view === 'data' ? 'Data sources' : view === 'users' ? 'Users & access' : view}</span>
          </header>
...
function AdminDashboard({ onNavigate }: { onNavigate: (v: View) => void }) {
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4"><div className="text-[11.5px] text-navy-500 dark:text-slate-400">Platform administration</div><div className="text-[20px] font-semibold tracking-tight">Admin console</div></div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <button onClick={() => onNavigate('data')} className="rounded-xl border border-navy-100 bg-white p-4 text-left hover:border-accent-300 dark:border-slate-800 dark:bg-[#211c33] dark:hover:border-accent-500/40">
          ...
          <div className="mt-2.5 text-[13.5px] font-semibold">Data sources & ingestion</div>
          <div className="text-[11.5px] text-navy-500 dark:text-slate-400">Connect Databricks/Postgres or upload a CSV — the accelerator runs on the active source.</div>
          <div className="mt-2 inline-flex items-center gap-1 text-[11.5px] font-medium text-accent-600 dark:text-accent-400">Manage data <ArrowRight className="h-3 w-3" /></div>
        </button>
        <button onClick={() => onNavigate('users')} className="rounded-xl border border-navy-100 bg-white p-4 text-left hover:border-accent-300 dark:border-slate-800 dark:bg-[#211c33] dark:hover:border-accent-500/40">
          ...
          <div className="mt-2.5 text-[13.5px] font-semibold">Users & entitlements</div>
          <div className="text-[11.5px] text-navy-500 dark:text-slate-400">Persona-based access — Plant Manager, Maintenance, Finance, Executive, Admin.</div>
          <div className="mt-2 inline-flex items-center gap-1 text-[11.5px] font-medium text-accent-600 dark:text-accent-400">Manage access <ArrowRight className="h-3 w-3" /></div>
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div><h1 className="text-[19px] font-semibold tracking-tight">Data sources & ingestion</h1><p className="text-[12.5px] text-navy-500 dark:text-slate-500">Upload a CSV with demand history — the engine classifies (FSN/XYZ/ABC), computes safety stock, reorder point &amp; EOQ, and the whole accelerator runs on it.</p></div>
        <button onClick={downloadTemplate} className="inline-flex items-center gap-1.5 rounded-lg border border-navy-200 px-3 py-1.5 text-[12px] font-medium text-navy-600 hover:bg-navy-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"><Download className="h-3.5 w-3.5" /> Download CSV template</button>
      </div>
      ...
      <div className="rounded-xl border border-navy-100 bg-white p-4 dark:border-slate-800 dark:bg-[#211c33]">
        <div className="mb-3 text-[13px] font-semibold">Upload inventory extract (CSV)</div>
        ...
        <div onClick={() => fileRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) onFile(f); }}
          ...
          <div className="text-[12.5px] font-medium">Drop a CSV here, or click to browse</div>
          <div className="text-[11px] text-navy-400 dark:text-slate-500">Core: material_id, description, plant, category, supplier, stock_value, coverage_days, ved · plus monthly demand columns (M1…M18 or 2024-01…)</div>
        </div>
        ...
        {parsed && (
          <div className="mt-4">
            ...
            {historyCols.length > 0 && (
              <div className="mb-3 inline-flex items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/[0.06] px-3 py-1.5 text-[11.5px] font-medium text-emerald-700 dark:text-emerald-400">
            ...
            {/* Preview */}
            <div className="mb-2 flex items-center gap-1.5 text-[11.5px] font-semibold text-navy-600 dark:text-slate-300"><Table className="h-3.5 w-3.5" /> Preview — {parsed.rows.length} rows, {parsed.headers.length} columns</div>
            ...
            {/* Mapping */}
            <div className="mt-4 mb-2 text-[11.5px] font-semibold text-navy-600 dark:text-slate-300">Map to inventory schema · {mappedCount}/{FIELDS.length} core mapped</div>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {FIELDS.map((f) => (
                <div key={f.key} className="flex items-center gap-2 rounded-lg border border-navy-100 px-2.5 py-1.5 dark:border-slate-800">
                  <span className="w-24 shrink-0 text-[11px] font-medium">{f.label}</span>
                  <ArrowRight className="h-3 w-3 shrink-0 text-navy-300" />
                  <select value={mapping[f.key] || ''} onChange={(e) => setMapping((m) => ({ ...m, [f.key]: e.target.value }))} className="min-w-0 flex-1 rounded-md border border-navy-200 bg-white px-2 py-1 text-[11px] outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              {!result && (
                <button onClick={runIngest} disabled={mappedCount < 3 || busy} className="inline-flex items-center gap-1.5 rounded-lg bg-accent-500 px-4 py-2 text-[12.5px] font-semibold text-white hover:bg-accent-600 disabled:opacity-50">
                  {busy ? <><Spinner className="h-3.5 w-3.5" /> Ingesting &amp; computing policy…</> : <>Ingest &amp; activate source</>}
                </button>
              )}
              {busy && <span className="text-[11.5px] text-navy-500 dark:text-slate-400">Classifying, sizing safety stock / ROP / EOQ across {parsed.rows.length.toLocaleString()} SKUs…</span>}
              {error && <span className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-rose-600 dark:text-rose-400"><AlertTriangle className="h-4 w-4" /> {error}</span>}
            </div>

            {result && (
              <div className="mt-3 rounded-xl border border-emerald-500/30 bg-emerald-500/[0.05] p-4">
                <div className="mb-2 flex items-center gap-2 text-[13px] font-semibold text-emerald-700 dark:text-emerald-400"><CheckCircle2 className="h-4.5 w-4.5" /> Source activated — the accelerator now runs on this data.</div>
                ...
                <button onClick={openWorkspace} disabled={opening} className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-accent-500 px-4 py-2 text-[12.5px] font-semibold text-white hover:bg-accent-600 disabled:opacity-60">
                ...
                {result.issues?.[0] && <div className="mt-2 text-[10.5px] text-navy-500 dark:text-slate-400">{result.issues[0]}</div>}
              </div>
            )}
          </div>
        )}
...
function Metric({ value, label }: { value: string; label: string }) {
  return (
    <div className="rounded-lg border border-navy-100 bg-white px-3 py-2 dark:border-slate-800 dark:bg-[#211c33]">
      ...
      <div className="mt-1 text-[10.5px] text-navy-400 dark:text-slate-500">{label}</div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-navy-100 bg-white p-3.5 dark:border-slate-800 dark:bg-[#211c33]">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0 flex-1"><div className="truncate text-[12.5px] font-semibold">{name}</div><div className="truncate text-[10.5px] text-navy-400 dark:text-slate-500">{detail}</div></div>
        <span className={`rounded-full px-2 py-0.5 text-[9.5px] font-semibold capitalize ${tone}`}>{status === 'none' ? '—' : status}</span>
      </div>
    </div>
  );
}
...
function UsersAccess() {
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4"><h1 className="text-[19px] font-semibold tracking-tight">Users & entitlements</h1><p className="text-[12.5px] text-navy-500 dark:text-slate-500">Persona-based access — each role sees a scoped vantage point and approves at its workflow stage.</p></div>
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-5">
        {PERSONAS.map((p) => <div key={p.id} className="rounded-xl border border-navy-100 bg-white p-3 dark:border-slate-800 dark:bg-[#211c33]"><div className="text-[12px] font-semibold">{p.name}</div><div className="mt-0.5 text-[10.5px] text-navy-400 dark:text-slate-500">{p.role}</div></div>)}
      </div>
      <div className="overflow-hidden rounded-xl border border-navy-100 dark:border-slate-800">
        <table className="w-full min-w-[640px] text-[12px]">
          <thead><tr className="border-b border-navy-100 bg-navy-50/60 text-left text-[10px] uppercase tracking-wide text-navy-400 dark:border-slate-800 dark:bg-slate-900/40 dark:text-slate-500"><th className="px-4 py-2.5 font-semibold">User</th><th className="px-3 py-2.5 font-semibold">Persona</th><th className="px-3 py-2.5 font-semibold">Scope</th><th className="px-3 py-2.5 font-semibold">Status</th></tr></thead>
          <tbody>
            {demoUsers.map((u) => (
              <tr key={u.email} className="border-b border-navy-50 last:border-0 dark:border-slate-800/60 dark:bg-[#211c33]">
                <td className="px-4 py-2.5"><div className="font-medium">{u.name}</div><div className="text-[10px] text-navy-400 dark:text-slate-500">{u.email}</div></td>
                <td className="px-3 py-2.5"><span className="rounded-full bg-accent-500/10 px-2 py-0.5 text-[10px] font-semibold text-accent-700 dark:text-accent-300">{u.persona}</span></td>
                <td className="px-3 py-2.5 text-navy-500 dark:text-slate-400">{u.plants}</td>
                <td className="px-3 py-2.5"><span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/15 px-2 py-0.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400"><CheckCircle2 className="h-3 w-3" /> Active</span></td>
              </tr>
            ))}
          </tbody>
        </table>
