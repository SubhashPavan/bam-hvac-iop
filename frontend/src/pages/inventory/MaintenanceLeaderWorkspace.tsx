          ...
          <header className="relative flex h-13 shrink-0 items-center gap-3 border-b border-navy-100 px-5 py-3 dark:border-slate-800/70">
            <div className="flex items-center gap-2.5">
              ...
              <div className="leading-tight">
                ...
                <div className="text-[8.5px] font-semibold uppercase tracking-[0.16em] text-accent-600 dark:text-accent-400">Maintenance Leader · Stage 2</div>
              </div>
            </div>
            <span className="mx-1 hidden h-5 w-px bg-navy-200 dark:bg-slate-700 sm:block" />
            <span className="hidden text-[13px] font-medium capitalize text-navy-500 dark:text-slate-400 sm:block">{view === 'requests' ? 'Incoming' : view}</span>
            <span className="ml-auto rounded-lg border border-navy-200 bg-white px-3 py-1.5 text-[12px] dark:border-slate-700 dark:bg-slate-900">All plants · India</span>
          </header>
...
function MaintenanceDashboard({ mats, selected, onNavigate }: { mats: typeof MATERIALS; selected: string[]; onNavigate: (v: View) => void }) {
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4">
        <div className="text-[11.5px] text-navy-500 dark:text-slate-400">Maintenance planning · <span className="font-medium text-accent-600 dark:text-accent-400">{incoming.length} requests awaiting your approval</span></div>
        <div className="text-[20px] font-semibold tracking-tight">Maintenance Leader cockpit</div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Panel title="Incoming for your approval" sub={`${incoming.length} routed from Plant Managers`}>
          {incoming.length === 0 ? (
            <div className="py-8 text-center text-[12.5px] text-navy-500 dark:text-slate-400">Nothing awaiting you. When a Plant Manager submits an action, it lands here.</div>
          ) : (
            <div className="flex flex-col gap-1.5">
              {incoming.slice(0, 8).map((r) => (
                <div key={r.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-navy-100 px-3 py-2 dark:border-slate-800">
                  ...
                  <div className="min-w-0 flex-1"><div className="truncate text-[12px] font-medium">{r.materialDesc}</div><div className="text-[10px] text-navy-400 dark:text-slate-500">{r.plantId} · {money(r.savings)}{r.snapshot ? ' · sim attached' : ''}</div></div>
                  <button onClick={() => decide(r.id, 'approved', STAGE_LABEL['maintenance' as Stage])} className="inline-flex items-center gap-1 rounded-md bg-emerald-500 px-2.5 py-1.5 text-[10.5px] font-semibold text-white hover:bg-emerald-600"><Check className="h-3 w-3" /> Approve</button>
              ...
              <button onClick={() => onNavigate('requests')} className="mt-1 text-[11.5px] font-medium text-accent-600 hover:underline dark:text-accent-400">Open full queue →</button>
            </div>
          )}
        </Panel>
      ...
      <Panel title="Work-order pipeline" sub="requests by stage" className="mt-4">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
          {(['maintenance', 'finance', 'regional', 'global', 'done'] as Stage[]).map((st) => {
            ...
            return <div key={st} className="rounded-xl border border-navy-100 p-3 text-center dark:border-slate-800"><div className="text-[22px] font-semibold">{count}</div><div className="text-[10.5px] text-navy-500 dark:text-slate-400">{st === 'maintenance' ? 'With me' : STAGE_LABEL[st]}</div></div>;
          })}
        </div>
      </Panel>
    </div>
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div><h1 className="text-[19px] font-semibold tracking-tight">Spare Readiness</h1><p className="text-[12.5px] text-navy-500 dark:text-slate-500">{ready} of {spares.length} vital/essential spares ready · lowest cover first</p></div>
        <div className="flex items-center gap-2 rounded-lg border border-navy-200 bg-white px-2.5 py-1.5 dark:border-slate-700 dark:bg-slate-900">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search spares…" className="w-48 border-none bg-transparent text-[12.5px] outline-none placeholder:text-navy-400 dark:text-slate-200" />
        </div>
      </div>
              ...
              {shown.slice(0, 40).map((m) => {
                ...
                return (
                  <tr key={m.id} className="border-b border-navy-50 last:border-0 hover:bg-navy-50/50 dark:border-slate-800/60 dark:bg-[#211c33] dark:hover:bg-slate-800/30">
                    <td className="px-4 py-2.5"><div className="font-medium">{m.description}</div><div className="font-mono text-[10px] text-navy-400 dark:text-slate-500">{m.id}</div></td>
                    <td className="px-3 py-2.5 text-navy-500 dark:text-slate-400">{m.plant_id}</td>
                    <td className="px-3 py-2.5"><span className={`rounded px-1.5 py-0.5 text-[9.5px] font-semibold ${m.ved === 'Vital' ? 'bg-rose-500/15 text-rose-600 dark:text-rose-400' : 'bg-amber-500/15 text-amber-600 dark:text-amber-400'}`}>{m.ved}</span> <span className="text-[10px] text-navy-400">{m.criticality_score}</span></td>
