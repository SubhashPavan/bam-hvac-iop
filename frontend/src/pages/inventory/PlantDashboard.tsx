  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <div>
          <h1 className="text-[19px] font-semibold tracking-tight">Plant Dashboard</h1>
          <p className="text-[12.5px] text-navy-500 dark:text-slate-500">Plant-level inventory detail · Critical spares · Dead stock</p>
        </div>
        <select value={plantId} onChange={(e) => setPlantId(e.target.value)}
          className="ml-auto rounded-lg border border-navy-200 bg-white px-3 py-1.5 text-[12.5px] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
          {plantList.map((p) => <option key={p.id} value={p.id}>{p.id} — {p.name}</option>)}
        </select>
        <span className="rounded-full bg-accent-500/10 px-2.5 py-1 text-[11.5px] font-medium text-accent-700 dark:text-accent-300">{mats.length} materials</span>
        <span className="flex items-center gap-1.5 text-[11.5px] text-navy-500 dark:text-slate-400"><span className={`h-2 w-2 rounded-full ${plant.service_level < 92 ? 'bg-rose-500' : 'bg-emerald-500'}`} /> Service Level ≈ {plant.service_level}%</span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-navy-100 bg-white p-4 dark:border-slate-800 dark:bg-[#211c33]">
          <div className="mb-2 text-[13px] font-semibold">Category Distribution</div>
        ...
        <div className="rounded-xl border border-navy-100 bg-white p-4 dark:border-slate-800 dark:bg-[#211c33]">
          <div className="mb-3 text-[13px] font-semibold">Critical Spares</div>
            ...
            {criticalSpares.map((m) => (
              <div key={m.id} className="flex items-center gap-3 rounded-lg border-l-2 border-amber-400 bg-navy-50/50 px-3 py-2 dark:bg-slate-800/40">
                <div className="min-w-0 flex-1">
                  <div className="font-mono text-[10px] text-navy-400 dark:text-slate-500">{m.id}</div>
                  <div className="truncate text-[12px] font-medium">{m.description}</div>
                </div>
                <div className="text-right">
                  <div className="text-[13px] font-semibold text-amber-600 dark:text-amber-400">{m.criticality_score}/100</div>
                  <div className="text-[10px] text-navy-400 dark:text-slate-500">{m.ved}</div>
                </div>
              </div>
            ))}
            {criticalSpares.length === 0 && <div className="py-6 text-center text-[12.5px] text-navy-500 dark:text-slate-400">No critical spares at this plant.</div>}
          </div>
        </div>
      </div>
    </div>
...
function Kpi({ icon: Icon, tone, value, label, sub }: { icon: typeof Box; tone: string; value: string; label: string; sub?: string }) {
  return (
    <div className="rounded-xl border border-navy-100 bg-white p-4 dark:border-slate-800 dark:bg-[#211c33]">
      ...
      {sub && <div className="text-[10.5px] text-navy-400 dark:text-slate-500">{sub}</div>}
    </div>
  );
}
