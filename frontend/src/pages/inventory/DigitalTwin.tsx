...
export default function DigitalTwin() {
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4">
        <h1 className="text-[20px] font-semibold tracking-tight">Digital Twin Simulator</h1>
        <p className="text-[14px] text-navy-500 dark:text-slate-500">What-if analysis · Scenario planning · Risk simulation</p>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between rounded-xl border border-accent-300/40 bg-accent-500/5 p-4 dark:border-accent-500/30 dark:bg-accent-500/10">
        <div>
          <div className="text-[14.5px] font-semibold text-accent-700 dark:text-accent-300">Digital Twin Mode — Advanced</div>
          <div className="text-[13px] text-navy-500 dark:text-slate-400">Simulate disruption scenarios and instantly see inventory, service level, and cost impacts without affecting live data.</div>
        </div>
        <span className="rounded-full bg-success-500/15 px-2.5 py-1 text-[12.5px] font-semibold text-success-600 dark:text-success-400">Simulation Ready</span>
      </div>

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {/* Scenario type */}
        <div className="rounded-xl border border-navy-100 bg-white p-4 dark:border-slate-800 dark:bg-[#211c33]">
          <div className="mb-3 text-[12.5px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Scenario Type</div>
            ...
            {SCENARIOS.map((s) => {
              ...
              return (
                <button key={s.key} onClick={() => { setScenario(s.key); setImpact(null); }}
                  className={`flex w-full items-center gap-2.5 rounded-lg border px-3 py-2.5 text-left text-[14px] font-medium transition-colors ${active ? 'border-accent-400 bg-accent-500/10 dark:border-accent-500' : 'border-navy-100 hover:bg-navy-50 dark:border-slate-800 dark:hover:bg-slate-800/40'}`}>
        ...
        {/* Parameters */}
        <div className="rounded-xl border border-navy-100 bg-white p-4 dark:border-slate-800 dark:bg-[#211c33]">
          <div className="mb-3 text-[12.5px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Parameters</div>
          <label className="mb-1 block text-[13px] font-medium">Plant</label>
          <select value={plant} onChange={(e) => setPlant(e.target.value)}
            className="mb-4 w-full rounded-lg border border-navy-200 bg-white px-3 py-2 text-[14px] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
            {PLANTS.map((p) => <option key={p.id} value={p.id}>{p.id} — {p.name}</option>)}
          </select>
          <div className="mb-1 flex items-center justify-between text-[13px] font-medium"><span>Intensity</span><span className="text-accent-600 dark:text-accent-400">{intensity}%</span></div>
          ...
          <div className="mb-4 flex justify-between text-[11.5px] text-navy-400 dark:text-slate-500"><span>Low</span><span>Severe</span></div>
          <button onClick={run} disabled={running}
            className="flex w-full items-center justify-center gap-2 rounded-lg bg-accent-500 px-3 py-2.5 text-[14px] font-semibold text-white hover:bg-accent-600 disabled:opacity-60">
        ...
        {/* Impact preview */}
        <div className="rounded-xl border border-navy-100 bg-white p-4 dark:border-slate-800 dark:bg-[#211c33]">
          <div className="mb-3 text-[12.5px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Impact Preview</div>
          {!impact && !running ? (
            <div className="flex h-[220px] flex-col items-center justify-center gap-2 text-center text-[13.5px] text-navy-400 dark:text-slate-500">
...
function Impact({ label, value, tone }: { label: string; value: string; tone: 'rose' | 'emerald' | 'amber' }) {
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-navy-100 bg-navy-50/50 px-3 py-2.5 dark:border-slate-800 dark:bg-slate-900/40">
      <span className="text-[13px] text-navy-600 dark:text-slate-400">{label}</span>
      <span className={`text-[16.5px] font-semibold ${c}`}>{value}</span>
    </div>
  );
}
