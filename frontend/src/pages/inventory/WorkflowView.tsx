...
export default function WorkflowView() {
  ...
  if (requests.length === 0) {
    return (
      <div className="px-6 py-16 text-center">
        ...
        <div className="mt-3 text-[15px] font-semibold">No actions in the workflow yet</div>
        <div className="mt-1 text-[12.5px] text-navy-500 dark:text-slate-400">Act on a recommendation, opportunity, or exception — it lands here as a request for approval.</div>
      </div>
    );
  }
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4">
        <h1 className="text-[19px] font-semibold tracking-tight">Approval workflow</h1>
        <p className="text-[12.5px] text-navy-500 dark:text-slate-500">Plant Manager → Maintenance Planner → Finance Controller → Regional → Global · approve, send back, or reject at your stage</p>
      </div>
      ...
      <div className="mb-3 flex flex-wrap gap-1.5">
        {(['all', 'awaiting', 'inflight', 'closed'] as Filter[]).map((f) => (
          <button key={f} onClick={() => setFilter(f)} className={`rounded-lg px-3 py-1.5 text-[12px] font-semibold capitalize ${filter === f ? 'bg-accent-500 text-white' : 'bg-navy-100 text-navy-600 hover:bg-navy-200 dark:bg-slate-800 dark:text-slate-300'}`}>{f === 'awaiting' ? 'Awaiting approval' : f === 'inflight' ? 'In flight' : f}</button>
        ))}
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-[12px]">
            ...
            <tbody>
              ...
              {shown.length === 0 && <tr><td colSpan={7} className="px-4 py-10 text-center text-[13px] text-navy-500 dark:text-slate-400">Nothing in this view.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <tr className="border-b border-navy-50 hover:bg-navy-50/50 dark:border-slate-800/60 dark:bg-[#211c33] dark:hover:bg-slate-800/30">
        <td className="px-4 py-2.5">
          <button onClick={onToggle} className="flex items-start gap-1.5 text-left">
            ...
            <span><span className="font-medium">{r.materialDesc}</span><span className="block font-mono text-[10px] text-navy-400 dark:text-slate-500">{r.id} · {r.plantId}</span></span>
          </button>
        </td>
        <td className="px-3 py-2.5"><span className="rounded-full bg-accent-500/10 px-2 py-0.5 text-[10px] font-semibold text-accent-700 dark:text-accent-300">{ACTION_LABEL[r.kind]}</span><span className={`ml-1 rounded-full px-1.5 py-0.5 text-[9px] font-semibold capitalize ${PRIO_TONE[r.priority]}`}>{r.priority}</span></td>
        ...
        <td className="px-3 py-2.5">{r.snapshot ? <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] font-semibold text-amber-600 dark:text-amber-400"><FlaskConical className="h-3 w-3" /> {r.snapshot.service}% svc</span> : <span className="text-[11px] text-navy-300 dark:text-slate-600">—</span>}</td>
        <td className="px-3 py-2.5"><span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${STAGE_PILL(r.stage)}`}>{STAGE_LABEL[r.stage]}</span></td>
        <td className="px-4 py-2.5">
          {closed ? (
            <span className={`inline-flex items-center gap-1 text-[11px] font-semibold ${r.stage === 'done' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>{r.stage === 'done' ? <><CheckCircle2 className="h-3.5 w-3.5" /> Done</> : <><XCircle className="h-3.5 w-3.5" /> Rejected</>}</span>
          ) : r.stage === 'executing' ? (
            <button onClick={onApprove} className="rounded-md bg-emerald-500/15 px-2.5 py-1.5 text-[11px] font-semibold text-emerald-600 hover:bg-emerald-500/25 dark:text-emerald-400">Mark done</button>
          ) : (
            <div className="flex items-center justify-end gap-1">
              <button onClick={onApprove} title={`Approve as ${actor}`} className="inline-flex items-center gap-1 rounded-md bg-emerald-500 px-2.5 py-1.5 text-[11px] font-semibold text-white hover:bg-emerald-600"><Check className="h-3.5 w-3.5" /> Approve</button>
          ...
          <td colSpan={7} className="px-6 py-3.5">
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              <div>
                    ...
                    return (
                      <div key={st} className="flex flex-1 items-center gap-1">
                        <div className="flex flex-col items-center gap-1">
                          <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[9px] font-bold ${done ? 'bg-emerald-500 text-white' : current ? 'bg-accent-500 text-white' : 'bg-navy-100 text-navy-400 dark:bg-slate-800 dark:text-slate-500'}`}>{done ? <Check className="h-3 w-3" /> : i + 1}</span>
                          <span className={`whitespace-nowrap text-[8.5px] ${current ? 'font-semibold text-accent-600 dark:text-accent-400' : 'text-navy-400 dark:text-slate-500'}`}>{STAGE_LABEL[st].split(' ')[0]}</span>
                        </div>
                ...
                <div className="text-[10px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">AI justification</div>
                <p className="text-[11.5px] leading-relaxed text-navy-600 dark:text-slate-300">{r.justification}</p>
                {r.note && <><div className="mt-2 text-[10px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Note</div><p className="text-[11.5px] text-navy-600 dark:text-slate-300">{r.note}</p></>}
              </div>

              <div>
                <div className="text-[10px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Scenario evidence</div>
                {r.snapshot ? (
                  <div className="mt-1 rounded-lg border border-amber-300/40 bg-amber-50/50 p-2.5 dark:border-amber-500/25 dark:bg-amber-500/5">
                    <div className="flex items-center gap-1.5 text-[11px] font-semibold text-amber-700 dark:text-amber-300"><FlaskConical className="h-3 w-3" /> {r.snapshot.scenario}</div>
                    <div className="mt-1 grid grid-cols-3 gap-2 text-[11px]">
                      <div><div className="text-[9px] uppercase text-navy-400 dark:text-slate-500">Service</div><b>{r.snapshot.service}%</b></div>
                      <div><div className="text-[9px] uppercase text-navy-400 dark:text-slate-500">Investment</div><b>{money(r.snapshot.investment)}</b></div>
                      <div><div className="text-[9px] uppercase text-navy-400 dark:text-slate-500">Stockout</div><b>{r.snapshot.stockoutRisk}%</b></div>
                    </div>
                  </div>
                ) : <div className="mt-1 rounded-lg border border-dashed border-navy-200 px-2.5 py-2 text-[11px] text-navy-400 dark:border-slate-700 dark:text-slate-500">No simulation attached.</div>}
                <div className="mt-2 text-[11px] text-navy-500 dark:text-slate-400">Cash release: <b className="text-emerald-600 dark:text-emerald-400">{money(r.cashRelease)}</b></div>
              </div>

              <div>
                <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Audit trail</div>
                <div className="flex flex-col gap-1.5">
                  {r.history.map((h, i) => (
                    <div key={i} className="flex gap-2">
                      ...
                      <div className="text-[11px]"><span className="font-medium">{h.actor}</span> <span className="text-navy-400 dark:text-slate-500">{h.decision === 'created' ? 'created & approved' : h.decision === 'sent_back' ? 'sent back' : h.decision} · {STAGE_LABEL[h.stage]}</span>{h.comment && <div className="text-navy-500 dark:text-slate-400">“{h.comment}”</div>}</div>
                    </div>
                  ))}
                </div>
              </div>
