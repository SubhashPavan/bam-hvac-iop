...
export default function NetworkPooling({ plantIds }: { plantIds: string[] }) {
  ...
  return (
    <div className="relative px-6 py-5">
      <TopProgressBar />
      <div className="mb-4">
        <h1 className="flex items-center gap-2 text-[19px] font-semibold tracking-tight"><Network className="h-5 w-5 text-accent-500" /> Network stock pooling</h1>
        <p className="text-[12.5px] text-navy-500 dark:text-slate-500">Cover shortages from surplus already sitting elsewhere in the network — instead of buying. Each move frees capital and protects a critical part.</p>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-navy-100 bg-white dark:border-slate-800 dark:bg-[#211c33]">
        <div className="flex items-center justify-between border-b border-navy-100 px-4 py-2.5 dark:border-slate-800">
          <span className="text-[12.5px] font-semibold">Recommended moves <span className="text-navy-400 dark:text-slate-500">· highest value first</span></span>
          <div className="flex items-center gap-2 rounded-lg border border-navy-200 bg-white px-2.5 py-1 dark:border-slate-700 dark:bg-slate-900">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search part, plant…" className="w-48 border-none bg-transparent text-[12px] outline-none placeholder:text-navy-400 dark:text-slate-200" />
          </div>
        </div>
          ...
          <table className="w-full min-w-[820px] text-[12px]">
            ...
            <tbody>{shown.map((m) => (
              <tr key={m.id} className="border-b border-navy-50 last:border-0 hover:bg-navy-50/60 dark:border-slate-800/60 dark:hover:bg-slate-800/40">
                <td className="px-4 py-2.5"><div className="font-medium">{m.part}</div><div className="text-[10px] text-navy-400 dark:text-slate-500">{m.category} · {m.supplier}</div></td>
                <td className="px-2.5 py-2.5">
                  <div className="flex items-center gap-1.5 text-[11.5px]">
                    <span className="rounded bg-emerald-500/10 px-1.5 py-0.5 text-[10px] font-medium text-emerald-600 dark:text-emerald-400">{m.from_plant_name}</span>
                    <ArrowRight className="h-3 w-3 text-navy-400" />
                    <span className="rounded bg-rose-500/10 px-1.5 py-0.5 text-[10px] font-medium text-rose-600 dark:text-rose-400">{m.to_plant_name}</span>
                  </div>
                  <div className="text-[9px] text-navy-400 dark:text-slate-500">cover {m.from_coverage}d → {m.to_coverage}d</div>
                </td>
                ...
                <td className="px-2.5 py-2.5"><span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${VED_TONE[m.to_ved] || ''}`}>{m.to_ved}</span></td>
                <td className="px-2 py-2.5 text-right">
                  <button onClick={() => openAction(m.action_seed as ActionSeed)} className="inline-flex items-center gap-1 rounded-lg bg-accent-500 px-2 py-1 text-[10.5px] font-semibold text-white hover:bg-accent-600"><Sparkles className="h-3 w-3" /> Rebalance</button>
                </td>
              </tr>
            ))}
            {moves.length === 0 && <tr><td colSpan={6} className="px-3 py-10 text-center text-[12.5px] text-navy-500 dark:text-slate-400">No pooling moves — no part family has surplus at one plant and a shortage at another in this scope.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
