    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex h-full w-[min(560px,95vw)] flex-col border-l border-navy-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-[#14111f]">
        ...
        <div className="flex items-center gap-2.5 border-b border-navy-100 px-5 py-3.5 dark:border-slate-800">
          ...
          <div className="min-w-0 flex-1"><div className="text-[14px] font-semibold leading-tight">Create action</div><div className="text-[11px] text-navy-400 dark:text-slate-500">Structured request · routes to the approval workflow</div></div>
        ...
        <div className="custom-scrollbar flex-1 overflow-y-auto px-5 py-4">
          ...
          <div className="rounded-xl border border-navy-100 p-3.5 dark:border-slate-800">
            <div className="flex items-center gap-2">
              <span className="rounded-full bg-accent-500/10 px-2 py-0.5 text-[10px] font-semibold text-accent-700 dark:text-accent-300">{ACTION_LABEL[draft.kind]}</span>
              <span className="text-[10px] text-navy-400 dark:text-slate-500">{draft.plantId}</span>
            </div>
            <div className="mt-1.5 text-[13.5px] font-semibold">{draft.materialDesc}</div>
            <div className="font-mono text-[10.5px] text-navy-400 dark:text-slate-500">{draft.materialId}</div>
          ...
          <div className="mt-3 rounded-xl bg-accent-500/5 p-3 dark:bg-accent-500/10">
            <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-accent-600 dark:text-accent-400">AI justification</div>
            <p className="text-[12px] leading-relaxed text-navy-700 dark:text-slate-300">{draft.justification}</p>
          </div>

          {/* Simulation evidence */}
          {draft.snapshot ? (
            <div className="mt-3 rounded-xl border border-amber-300/50 bg-amber-50/60 p-3 dark:border-amber-500/25 dark:bg-amber-500/5">
              <div className="mb-1.5 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wide text-amber-600 dark:text-amber-400"><FlaskConical className="h-3 w-3" /> Simulation attached as evidence</div>
              <div className="text-[12px] font-medium">{draft.snapshot.scenario}</div>
              <div className="mt-1 flex flex-wrap gap-x-4 gap-y-0.5 text-[11px] text-navy-600 dark:text-slate-300">
            ...
            <div className="mt-3 flex items-center gap-2 rounded-xl border border-dashed border-navy-200 px-3 py-2.5 text-[11.5px] text-navy-500 dark:border-slate-700 dark:text-slate-400"><FlaskConical className="h-3.5 w-3.5" /> No simulation attached. Run one on the Simulation tab to attach it as evidence.</div>
          )}

          {/* Priority */}
          <div className="mt-4">
            <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Priority</div>
            <div className="flex gap-1.5">
              {PRIORITIES.map((p) => (
                <button key={p.k} onClick={() => setPriority(p.k)} className={`flex-1 rounded-lg border px-2 py-1.5 text-[11.5px] font-semibold ${priority === p.k ? 'border-accent-400 bg-accent-500/10 dark:border-accent-500' : 'border-navy-200 hover:bg-navy-50 dark:border-slate-700 dark:hover:bg-slate-800'} ${priority === p.k ? p.tone : 'text-navy-500 dark:text-slate-400'}`}>{p.label === 'Urgent' && priority === p.k ? <span className="inline-flex items-center gap-1"><AlertTriangle className="h-3 w-3" />{p.label}</span> : p.label}</button>
              ))}
            </div>
          </div>
          ...
          {/* Note */}
          <div className="mt-4">
            <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Note to planner (optional)</div>
            <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2} placeholder="Add context for the reviewer…" className="w-full resize-none rounded-lg border border-navy-200 bg-white px-3 py-2 text-[12.5px] outline-none focus:border-accent-400 focus:ring-2 focus:ring-accent-500/20 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" />
          </div>

          {/* Routing */}
          <div className="mt-4">
            <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Route</div>
            <div className="flex flex-col gap-1.5">
              {ROUTES.map((r) => (
                <button key={r.k} onClick={() => setRouting(r.k)} className={`flex items-center gap-2.5 rounded-lg border px-3 py-2 text-left ${routing === r.k ? 'border-accent-400 bg-accent-500/10 dark:border-accent-500' : 'border-navy-200 hover:bg-navy-50 dark:border-slate-700 dark:hover:bg-slate-800'}`}>
                  ...
                  <div className="min-w-0 flex-1"><div className="text-[12.5px] font-medium">{r.label}</div><div className="text-[10.5px] text-navy-400 dark:text-slate-500">{r.desc}</div></div>
        ...
        <div className="flex items-center gap-2 border-t border-navy-100 px-5 py-3.5 dark:border-slate-800">
          <div className="text-[11px] text-navy-400 dark:text-slate-500">Stage 1 of 5 · you originate & approve</div>
          <button onClick={closeAction} className="ml-auto rounded-lg border border-navy-200 px-3.5 py-2 text-[12px] font-medium text-navy-600 hover:bg-navy-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Cancel</button>
          <button onClick={submit} className="inline-flex items-center gap-1.5 rounded-lg bg-accent-500 px-4 py-2 text-[12.5px] font-semibold text-white hover:bg-accent-600">Submit action <ArrowRight className="h-3.5 w-3.5" /></button>
        </div>
      </div>
    </>
  );
