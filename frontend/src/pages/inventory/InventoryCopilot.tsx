  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-navy-100 px-6 py-4 dark:border-slate-800">
        <h1 className="text-[19px] font-semibold tracking-tight">Ask</h1>
        <p className="text-[12.5px] text-navy-500 dark:text-slate-500">Conversational inventory intelligence · Inventory Optimization Assistant</p>
      </div>
        ...
        <div className="mx-auto max-w-3xl">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {QUICK.map((q) => (
                <button key={q} onClick={() => send(q)}
                  className="rounded-full border border-navy-200 bg-white px-3 py-1.5 text-[11.5px] text-navy-600 hover:border-accent-300 hover:bg-accent-500/5 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800">{q}</button>
              ))}
            </div>
          )}
          ...
          {msgs.length === 0 && (
            <div className="flex gap-2.5">
              <Avatar />
              <div className="rounded-2xl rounded-tl-sm border border-navy-100 bg-white px-4 py-3 text-[13px] leading-relaxed dark:border-slate-800 dark:bg-[#211c33]">
          ...
          <div className="flex flex-col gap-5">
            {msgs.map((m) => m.role === 'user' ? (
              <div key={m.id} className="max-w-[78%] self-end rounded-2xl rounded-br-sm bg-accent-500/10 px-3.5 py-2 text-[12.5px] leading-relaxed text-accent-800 dark:bg-accent-500/15 dark:text-accent-200">{m.text}</div>
            ) : (
                ...
                <div className="min-w-0 flex-1">
                  {m.tools && (
                    <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
                      <span className="text-[9.5px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Tools</span>
                      {m.tools.map((t) => <span key={t} className="flex items-center gap-1 rounded border border-navy-200 bg-white px-1.5 py-0.5 font-mono text-[10px] text-navy-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"><Database className="h-2.5 w-2.5" />{t}</span>)}
                    </div>
                  )}
                  <div className="text-[12.5px] leading-relaxed text-navy-800 dark:text-slate-200">{m.text}</div>
                  {m.sources && (
                    <div className="mt-2 flex items-center gap-2 border-t border-dashed border-navy-100 pt-1.5 text-[10px] text-navy-400 dark:border-slate-800 dark:text-slate-500">
            ...
            {thinking && (
              <div className="flex gap-2.5"><Avatar />
                <div className="rounded-2xl border border-navy-100 bg-white px-3.5 py-2.5 text-[12px] text-navy-500 dark:border-slate-800 dark:bg-[#211c33] dark:text-slate-400">Querying gold tables…</div>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="border-t border-navy-100 px-6 py-3 dark:border-slate-800">
        <form onSubmit={(e) => { e.preventDefault(); send(input); }} className="mx-auto flex max-w-3xl items-center gap-2 rounded-xl border border-navy-200 bg-white px-3 py-2 focus-within:border-accent-400 dark:border-slate-700 dark:bg-slate-900">
          <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask anything about your MRO inventory…"
            className="flex-1 border-none bg-transparent text-[12.5px] outline-none placeholder:text-navy-400 dark:text-slate-200 dark:placeholder:text-slate-500" />
