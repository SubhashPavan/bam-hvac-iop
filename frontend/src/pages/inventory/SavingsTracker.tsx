...
export default function SavingsTracker({ recs, selected }: { recs: Recommendation[]; selected: string[] }) {
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4">
        <h1 className="text-[19px] font-semibold tracking-tight">Savings Tracker</h1>
        <p className="text-[12.5px] text-navy-500 dark:text-slate-500">Actions taken · realized savings · financial impact · by opportunity & geography</p>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Panel title="Actions taken" sub={`${requests.length} requests from this workspace`}>
          {requests.length === 0 ? (
            <div className="py-10 text-center text-[12.5px] text-navy-500 dark:text-slate-400">No actions yet — act on a recommendation or opportunity to start tracking realized savings here.</div>
          ) : (
            <>
            <div className="mb-2 flex items-center gap-2 rounded-lg border border-navy-200 bg-white px-2.5 py-1 dark:border-slate-700 dark:bg-slate-900 sm:w-64">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search actions…" className="w-full border-none bg-transparent text-[12px] outline-none placeholder:text-navy-400 dark:text-slate-200" />
            </div>
            <div className="custom-scrollbar flex max-h-[240px] flex-col gap-1.5 overflow-y-auto pr-1">
              {shownReqs.map((r) => (
                <div key={r.id} className="flex items-center gap-3 rounded-lg border border-navy-100 px-3 py-2 dark:border-slate-800">
                  ...
                  <div className="min-w-0 flex-1"><div className="truncate text-[12px] font-medium">{r.materialDesc}</div><div className="text-[10px] text-navy-400 dark:text-slate-500">{r.plantId} · {money(r.savings)}</div></div>
                  <span className={`rounded-full px-2 py-0.5 text-[9.5px] font-semibold ${STAGE_TONE[r.stage] || 'bg-accent-500/15 text-accent-600 dark:text-accent-400'}`}>{STAGE_LABEL[r.stage]}</span>
                </div>
              ))}
              {shownReqs.length === 0 && <div className="py-4 text-center text-[11.5px] text-navy-500 dark:text-slate-400">No matching actions.</div>}
            </div>
            </>
          )}
        </Panel>
