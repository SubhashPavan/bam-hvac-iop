      ...
      <div className="flex h-screen overflow-hidden bg-navy-50 font-sans text-navy-900 dark:bg-[#14111f] dark:text-slate-100">
        ...
        <nav className={`flex shrink-0 flex-col gap-1 overflow-hidden border-r border-navy-100 bg-white py-3 transition-[width] duration-200 dark:border-slate-800/70 dark:bg-[#1b1730] ${navOpen ? 'w-[212px] items-stretch px-2.5' : 'w-[52px] items-center'}`}>
          ...
          <div className={`mt-auto flex flex-col gap-1 ${navOpen ? 'items-stretch' : 'items-center'}`}>
            <button onClick={() => setDark((v) => !v)} className={`flex items-center rounded-lg text-navy-400 hover:bg-navy-50 dark:text-slate-400 dark:hover:bg-slate-800 ${navOpen ? 'h-9 w-full gap-2.5 px-2.5' : 'h-9 w-9 justify-center'}`} aria-label="Theme">{dark ? <Sun style={{ width: 18, height: 18 }} className="shrink-0" /> : <Moon style={{ width: 18, height: 18 }} className="shrink-0" />}{navOpen && <span className="text-[12.5px] font-medium">{dark ? 'Light mode' : 'Dark mode'}</span>}</button>
            <button onClick={() => navigate('/accelerators')} className={`flex items-center rounded-lg text-navy-400 hover:bg-navy-50 dark:text-slate-400 dark:hover:bg-slate-800 ${navOpen ? 'h-9 w-full gap-2.5 px-2.5' : 'h-9 w-9 justify-center'}`} aria-label="Exit"><LogOut style={{ width: 18, height: 18 }} className="shrink-0" />{navOpen && <span className="text-[12.5px] font-medium">Exit accelerator</span>}</button>
          </div>
        </nav>
          ...
          <header className="relative flex h-13 shrink-0 items-center gap-3 border-b border-navy-100 px-5 py-3 dark:border-slate-800/70">
            <div className="flex items-center gap-2.5">
              ...
              <div className="leading-tight">
                ...
                <div className="text-[8.5px] font-semibold uppercase tracking-[0.16em] text-accent-600 dark:text-accent-400">Powered by Agentic AI</div>
              </div>
            </div>
            <span className="mx-1 hidden h-5 w-px bg-navy-200 dark:bg-slate-700 sm:block" />
            <span className="hidden text-[13px] font-medium capitalize text-navy-500 dark:text-slate-400 sm:block">{view}</span>
            <div className="ml-auto flex items-center gap-2">
              <div className="relative">
              <button onClick={() => setPlantOpen((v) => !v)} className="flex items-center gap-2 rounded-lg border border-navy-200 bg-white px-3 py-1.5 text-[12px] dark:border-slate-700 dark:bg-slate-900">
                ...
                <>
                  ...
                  <div className="absolute right-0 z-20 mt-1 w-56 rounded-xl border border-navy-100 bg-white p-1.5 shadow-lg dark:border-slate-700 dark:bg-slate-900">
                    {myPlants.map((p) => {
                      ...
                      return (
                        <button key={p.id} onClick={() => togglePlant(p.id)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[12.5px] hover:bg-navy-50 dark:hover:bg-slate-800">
        ...
        {toast && (
          <div className="fixed bottom-5 left-1/2 z-50 -translate-x-1/2 rounded-xl bg-emerald-600 px-4 py-2.5 text-[12.5px] font-medium text-white shadow-lg">
...
function NavIcon({ icon: Icon, active, onClick, label, expanded }: { icon: typeof LayoutDashboard; active: boolean; onClick: () => void; label: string; expanded?: boolean }) {
  return (
    <button onClick={onClick} title={expanded ? undefined : label} className={`flex items-center rounded-lg ${expanded ? 'h-9 w-full gap-2.5 px-2.5' : 'h-9 w-9 justify-center'} ${active ? 'bg-accent-500/10 text-accent-600 dark:bg-accent-500/15 dark:text-accent-300' : 'text-navy-400 hover:bg-navy-50 dark:text-slate-500 dark:hover:bg-slate-800'}`}>
      ...
      {expanded && <span className="truncate text-[12.5px] font-medium">{label}</span>}
    </button>
  );
}
  ...
  const renderBlock = (b: SageBlock, key: number) => {
    if (b.kind === 'kpis') return (
      <div key={key} className="grid grid-cols-3 gap-1.5">
        {b.tiles.map((t) => <div key={t.label} className="rounded-lg border border-navy-100 bg-white px-2 py-1.5 dark:border-slate-800 dark:bg-[#0e1730]"><div className={`text-[13px] font-semibold leading-none ${t.tone ? KPI_TONE[t.tone] : ''}`}>{t.value}</div><div className="mt-1 text-[9px] text-navy-400 dark:text-slate-500">{t.label}</div></div>)}
      </div>
    );
    if (b.kind === 'oppGroups') return (
      <div key={key} className="grid grid-cols-2 gap-1.5">
        {b.groups.map((g) => <button key={g.key} onClick={() => send(`show ${g.label}`)} className="rounded-lg border border-navy-100 bg-white p-2 text-left hover:border-accent-300 dark:border-slate-800 dark:bg-[#0e1730] dark:hover:border-accent-500/40"><div className="text-[10.5px] font-semibold">{g.label}</div><div className="text-[9px] text-navy-400 dark:text-slate-500">{g.count} · {sageMoney(g.value)}</div><div className="text-[10px] font-semibold text-amber-600 dark:text-amber-400">{sageMoney(g.savings)} savings</div></button>)}
      </div>
    );
    if (b.kind === 'items') return (
      <div key={key} className="rounded-xl border border-navy-100 bg-white p-2 dark:border-slate-800 dark:bg-[#0e1730]">
        <div className="mb-1 px-1 text-[10px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">{b.title}</div>
        <div className="flex flex-col gap-1">
          {b.items.map((it, i) => (
            <div key={i} className="flex items-center gap-2 rounded-lg px-1.5 py-1 hover:bg-navy-50 dark:hover:bg-slate-800/50">
              <div className="min-w-0 flex-1"><div className="truncate text-[11.5px] font-medium">{it.seed.materialDesc}</div><div className="text-[9.5px] text-navy-400 dark:text-slate-500">{it.badge} · {it.sub}</div></div>
              <span className="text-[11px] font-semibold text-amber-600 dark:text-amber-400">{sageMoney(it.value)}</span>
              <button onClick={() => openAction(it.seed)} className="rounded-md bg-accent-500 px-2 py-1 text-[10px] font-semibold text-white hover:bg-accent-600">{it.act}</button>
            </div>
          ))}
        </div>
      </div>
    ...
    if (b.kind === 'scenario') return (
      <div key={key} className="rounded-xl border border-amber-300/50 bg-amber-50/60 p-2.5 dark:border-amber-500/25 dark:bg-amber-500/5">
        <div className="mb-1 flex items-center gap-1.5 text-[9.5px] font-semibold uppercase tracking-wide text-amber-600 dark:text-amber-400"><Zap className="h-3 w-3" /> What-if simulation</div>
        <div className="text-[12px] font-semibold">{b.title}</div>
        <div className="mt-1 text-amber-500"><MiniChart series={[{ color: '#f59e0b', points: b.points }]} area height={54} /></div>
        <div className="mt-1.5 grid grid-cols-2 gap-1.5">
          {b.stats.map((st) => <div key={st.label} className="rounded-lg bg-white/70 px-2 py-1 dark:bg-slate-900/40"><div className="text-[9px] uppercase tracking-wide text-navy-400 dark:text-slate-500">{st.label}</div><div className={`text-[12px] font-semibold ${st.tone ? KPI_TONE[st.tone] : ''}`}>{st.value}</div></div>)}
        </div>
      </div>
    );
    if (b.kind === 'forecast') return (
      <div key={key} className="rounded-xl border border-navy-100 bg-white p-2 dark:border-slate-800 dark:bg-[#0e1730]">
        <div className="mb-1.5 grid grid-cols-3 gap-1.5">
          <div className="rounded-lg bg-emerald-500/10 px-2 py-1"><div className="text-[13px] font-semibold text-emerald-600 dark:text-emerald-400">{b.growth}</div><div className="text-[9px] text-navy-400 dark:text-slate-500">rising</div></div>
          <div className="rounded-lg bg-rose-500/10 px-2 py-1"><div className="text-[13px] font-semibold text-rose-600 dark:text-rose-400">{b.decline}</div><div className="text-[9px] text-navy-400 dark:text-slate-500">falling</div></div>
          <div className="rounded-lg bg-accent-500/10 px-2 py-1"><div className="text-[13px] font-semibold text-accent-600 dark:text-accent-400">{b.accuracy}%</div><div className="text-[9px] text-navy-400 dark:text-slate-500">accuracy</div></div>
        </div>
        <div className="flex flex-col gap-1">
          {b.movers.map((mv, i) => <div key={i} className="flex items-center gap-2 text-[11px]"><span className={mv.changePct >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}>{mv.changePct >= 0 ? '▲' : '▼'}</span><span className="min-w-0 flex-1 truncate">{mv.desc}</span><span className="text-navy-400 dark:text-slate-500">{mv.plant}</span><span className={`font-semibold ${mv.changePct >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>{mv.changePct >= 0 ? '+' : ''}{mv.changePct}%</span></div>)}
        </div>
      </div>
    );
    return null;
    ...
    <>
      {!open && (
        <button onClick={() => setOpen(true)} className="group fixed bottom-5 right-5 z-40 flex items-center gap-2 rounded-full bg-gradient-to-br from-accent-400 to-accent-600 py-3 pl-3.5 pr-4 text-white shadow-lg shadow-accent-500/40 ring-1 ring-white/20 transition-all hover:scale-105 hover:shadow-accent-500/60">
          ...
          <Sparkles className="h-5 w-5" /><span className="text-[13px] font-semibold">Sage</span>
          {emergencies.length > 0 && <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-bold ring-2 ring-white dark:ring-[#080d1a]">{emergencies.length}</span>}
        </button>
      )}
        ...
        <div className={`fixed inset-y-0 right-0 z-40 flex h-full flex-col overflow-hidden border-l border-navy-200 bg-white shadow-2xl dark:border-white/10 dark:bg-[#0a1020] ${wide ? 'w-[min(1040px,96vw)]' : 'w-[min(720px,94vw)]'} transition-[width] duration-200`}>
          ...
          <div className="relative flex items-center gap-3 border-b border-navy-100 bg-gradient-to-r from-accent-500/10 via-transparent to-transparent px-4 py-3.5 dark:border-white/10">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5 text-[14px] font-semibold leading-tight">Sage <span className="rounded-full bg-accent-500/15 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-accent-600 dark:text-accent-300">Optimization AI</span></div>
              <div className="flex items-center gap-1.5 text-[11px] text-navy-400 dark:text-slate-500"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> Queries your whole stock base · acts into the workflow</div>
            </div>
          ...
          <div ref={scroll} className="custom-scrollbar relative flex-1 overflow-y-auto px-4 py-4">
            <div className="mx-auto flex max-w-[760px] flex-col gap-3">
              {msgs.map((m) => m.role === 'user' ? (
                <div key={m.id} className="max-w-[80%] self-end rounded-2xl rounded-br-sm bg-accent-500/10 px-3.5 py-2 text-[12.5px] text-accent-800 dark:bg-accent-500/15 dark:text-accent-100">{m.text}</div>
              ) : (
                <div key={m.id} className="flex flex-col gap-2">
                  {m.reply.steps && (
                    <div className="rounded-xl border border-navy-100 bg-navy-50/60 p-2.5 dark:border-slate-800 dark:bg-slate-900/40">
                      <div className="mb-1 flex items-center gap-1.5 text-[9.5px] font-semibold uppercase tracking-wide text-accent-600 dark:text-accent-400"><Zap className="h-3 w-3" /> Working</div>
                      {m.reply.steps.map((st, i) => <div key={i} className="flex items-center gap-2 text-[11.5px] text-navy-700 dark:text-slate-300"><span className="flex h-4 w-4 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-600 dark:text-emerald-400"><Check className="h-2.5 w-2.5" /></span>{st}</div>)}
                    </div>
                  )}
                  <div className="max-w-[95%] rounded-2xl rounded-tl-sm bg-navy-50 px-3.5 py-2 text-[12.5px] leading-relaxed text-navy-700 dark:bg-slate-800 dark:text-slate-200"><RichText text={m.reply.text} /></div>
                  {m.reply.blocks.map((b, i) => renderBlock(b, i))}
                  {m.reply.chips && (
                    <div className="flex flex-wrap gap-1.5">
                      {m.reply.chips.map((c) => <button key={c} onClick={() => submit(c)} className="rounded-full border border-navy-200 px-2.5 py-1 text-[10.5px] font-medium text-navy-600 hover:border-accent-300 hover:bg-accent-500/5 hover:text-accent-700 dark:border-slate-700 dark:text-slate-300 dark:hover:text-accent-300">{c}</button>)}
                    </div>
                  )}
                </div>
              ))}
          ...
          <div className="relative border-t border-navy-100 p-3 dark:border-white/10">
            <div className="mx-auto max-w-[760px]">
              <div className="mb-2 flex flex-wrap items-center gap-1.5">
                <button onClick={() => runDeep(undefined, 'full_review')} className="inline-flex items-center gap-1 rounded-full border border-accent-500/40 bg-accent-500/10 px-3 py-1.5 text-[11px] font-semibold text-accent-600 hover:bg-accent-500/15 dark:text-accent-400"><Brain className="h-3.5 w-3.5" /> Full inventory review</button>
                {globalChips.map((c) => <button key={c} onClick={() => submit(c)} className="rounded-full border border-navy-200 bg-white px-3 py-1.5 text-[11px] font-medium text-navy-600 transition-colors hover:border-accent-300 hover:bg-accent-500/5 hover:text-accent-700 dark:border-slate-700 dark:bg-white/5 dark:text-slate-300 dark:hover:text-accent-300">{c}</button>)}
              </div>
              <form onSubmit={(e) => { e.preventDefault(); submit(); }} className="flex items-center gap-2 rounded-xl border border-navy-200 bg-white px-3.5 py-2.5 transition-shadow focus-within:border-accent-400 focus-within:ring-2 focus-within:ring-accent-500/20 dark:border-slate-700 dark:bg-white/5">
                <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask Sage — a question, an optimization, or “review my whole portfolio”…" className="min-w-0 flex-1 border-none bg-transparent text-[12.5px] outline-none placeholder:text-navy-400 dark:text-slate-200 dark:placeholder:text-slate-500" />
