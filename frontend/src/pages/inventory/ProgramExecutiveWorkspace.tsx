        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="relative flex h-13 shrink-0 items-center gap-3 border-b border-navy-100 px-5 py-3 dark:border-slate-800/70">
            <div className="flex items-center gap-2.5">
              ...
              <div className="leading-tight"><div className="text-[14.5px] font-semibold tracking-tight">Inventory Optimization</div><div className="text-[9.5px] font-semibold uppercase tracking-[0.16em] text-accent-600 dark:text-accent-400">Program Executive · Global</div></div>
            </div>
            <span className="mx-1 hidden h-5 w-px bg-navy-200 dark:bg-slate-700 sm:block" />
            <span className="hidden text-[14.5px] font-medium capitalize text-navy-500 dark:text-slate-400 sm:block">{view === 'network' ? 'Plant network' : view}</span>
            <span className="ml-auto rounded-lg border border-navy-200 bg-white px-3 py-1.5 text-[13.5px] dark:border-slate-700 dark:bg-slate-900">{PLANTS.length} plants · {new Set(PLANTS.map((p) => p.region)).size} regions</span>
          </header>
...
function ExecDashboard({ onNavigate }: { onNavigate: (v: View) => void }) {
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4">
        <div className="text-[13px] text-navy-500 dark:text-slate-400">Global program · <span className="font-medium text-accent-600 dark:text-accent-400">{money(ft.savings)} savings identified network-wide</span></div>
        <div className="text-[21px] font-semibold tracking-tight">Program Executive cockpit</div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Panel title="Executive sign-off" sub={`${incoming.length} escalated for regional/global approval`}>
          {incoming.length === 0 ? (
            <div className="py-8 text-center text-[14px] text-navy-500 dark:text-slate-400">Nothing escalated. High-value actions surface here after finance sign-off.</div>
          ) : (
            <div className="flex flex-col gap-1.5">
              {incoming.slice(0, 8).map((r) => (
                <div key={r.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-navy-100 px-3 py-2 dark:border-slate-800">
                  ...
                  <div className="min-w-0 flex-1"><div className="truncate text-[13.5px] font-medium">{r.materialDesc}</div><div className="text-[11.5px] text-navy-400 dark:text-slate-500">{r.plantId} · {money(r.savings)} · {STAGE_LABEL[r.stage]}</div></div>
                  <button onClick={() => decide(r.id, 'approved', actor(r.stage))} className="inline-flex items-center gap-1 rounded-md bg-emerald-500 px-2.5 py-1.5 text-[12px] font-semibold text-white hover:bg-emerald-600"><Check className="h-3 w-3" /> Approve</button>
...
function Portfolio() {
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4"><h1 className="text-[20px] font-semibold tracking-tight">Portfolio</h1><p className="text-[14px] text-navy-500 dark:text-slate-500">Savings program across every region</p></div>
      <Panel title="Regional program rollup">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[640px] text-[13.5px]">
            <thead><tr className="border-b border-navy-100 text-left text-[11.5px] uppercase tracking-wide text-navy-400 dark:border-slate-800 dark:text-slate-500"><th className="px-3 py-2 font-semibold">Region</th><th className="px-3 py-2 text-right font-semibold">Inventory</th><th className="px-3 py-2 text-right font-semibold">Savings</th><th className="px-3 py-2 text-right font-semibold">WC release</th><th className="px-3 py-2 text-right font-semibold">ROI</th><th className="px-3 py-2 font-semibold">Priority</th></tr></thead>
            <tbody>
              {regions.map((r) => (
