      ...
      <table className="w-full min-w-[1160px] text-[12px]">
        ...
        <tbody>{mats.slice(0, limit).map((m) => { const s = sumById.get(m.id); const st = statusOf(m, s); return (
          <tr key={m.id} onClick={() => onOpen(m)} className="cursor-pointer border-b border-navy-50 last:border-0 hover:bg-navy-50/60 dark:border-slate-800/60 dark:hover:bg-slate-800/40">
            <td className="px-4 py-2.5"><div className="font-medium">{m.description}</div><div className="font-mono text-[10px] text-navy-400 dark:text-slate-500">{m.id} · {m.plant_id} · {m.xyz}·{m.fsn[0]}·{m.ved[0]}</div></td>
            <td className={`px-2.5 py-2.5 capitalize ${PATTERN_TONE[s?.demand_pattern || ''] || 'text-navy-400 dark:text-slate-500'}`}>{s?.demand_pattern?.replace('_', ' ') || '—'}<div className="text-[9px] text-navy-400 dark:text-slate-500">{s?.method || ''}</div></td>
            <td className="px-2.5 py-2.5 text-right tabular-nums">{s ? <>{s.per_period_demand.toFixed(1)}<div className={`text-[9px] ${s.trend_pct >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>{s.trend_pct >= 0 ? '+' : ''}{s.trend_pct}%</div></> : '—'}</td>
            <td className="px-2.5 py-2.5 text-right tabular-nums">{m.on_hand_qty}<div className="text-[9px] text-navy-400 dark:text-slate-500">{money(m.current_stock_value)}</div></td>
            ...
            <td className="px-2.5 py-2.5 text-right tabular-nums">{m.coverage_days}d<div className="text-[9px] text-navy-400 dark:text-slate-500">→ {s?.recommended_coverage_days ?? '—'}d</div></td>
            ...
            <td className="px-2.5 py-2.5"><span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${st.tone}`}>{st.label}</span></td>
            <td className="px-2 py-2.5 text-right"><ChevronRight className="ml-auto h-4 w-4 text-navy-300 dark:text-slate-600" /></td>
          </tr>); })}
          {mats.length === 0 && <tr><td colSpan={13} className="px-3 py-10 text-center text-[12.5px] text-navy-500 dark:text-slate-400">No materials match.</td></tr>}
        </tbody>
      </table>
    </div></div>
  );
