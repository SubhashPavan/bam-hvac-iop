      ...
      <div onClick={(e) => e.stopPropagation()} className="custom-scrollbar h-full w-full max-w-[720px] overflow-y-auto border-l border-navy-100 bg-navy-50 shadow-2xl dark:border-slate-800 dark:bg-[#14111f]">
        ...
        <div className="sticky top-0 z-10 flex items-center gap-2.5 border-b border-navy-100 bg-white/90 px-5 py-3.5 backdrop-blur dark:border-slate-800 dark:bg-[#1b1730]/90">
          ...
          <div className="min-w-0 flex-1">
            <div className="truncate text-[14px] font-semibold">{report?.title || 'Deep insight'}</div>
            <div className="truncate text-[11px] text-navy-400 dark:text-slate-500">{report?.objective || 'Multi-step analysis'}</div>
          </div>
          {report && <span className="rounded-full bg-accent-500/10 px-2 py-0.5 text-[9.5px] font-semibold uppercase tracking-wide text-accent-600 dark:text-accent-400">{report.engine === 'llm' ? 'AI report' : 'report'}</span>}
        ...
        <div className="px-5 py-4">
          ...
          {running && (
            <div className="mb-4 rounded-xl border border-navy-100 bg-white p-4 dark:border-slate-800 dark:bg-[#211c33]">
              <div className="mb-2.5 flex items-center gap-2 text-[12.5px] font-semibold"><Spinner /> Running the analysis…</div>
          ...
          {report && (
            <>
              ...
              <div className="mb-4 rounded-xl border border-accent-500/25 bg-gradient-to-br from-accent-500/[0.07] to-transparent p-4">
                <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-accent-600 dark:text-accent-400"><Sparkles className="h-3.5 w-3.5" /> Executive summary</div>
                <p className="text-[13px] leading-relaxed text-navy-700 dark:text-slate-200">{report.narrative}</p>
              </div>

              {/* sections */}
              <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Analysis</div>
                  ...
                  return (
                    <div key={s.id} className="rounded-xl border border-navy-100 bg-white p-3.5 dark:border-slate-800 dark:bg-[#211c33]">
                      <div className="mb-1 flex items-center gap-2">
                        <span className="flex h-5 w-5 items-center justify-center rounded-md bg-navy-100 text-[10px] font-bold text-navy-500 dark:bg-slate-700 dark:text-slate-300">{i + 1}</span>
                        <span className="text-[12.5px] font-semibold">{s.title}</span>
                      </div>
                      <p className="text-[11.5px] leading-relaxed text-navy-600 dark:text-slate-400">{s.finding}</p>
                      {chips.length > 0 && (
                        <div className="mt-2.5 flex flex-wrap gap-1.5">
                          {chips.map((c, j) => (
                            <span key={j} className="inline-flex items-baseline gap-1 rounded-md border border-navy-100 bg-navy-50/60 px-2 py-1 text-[10.5px] dark:border-slate-700 dark:bg-slate-900/40">
              ...
              {report.actions.length > 0 && (
                <>
                  <div className="mb-2 mt-4 text-[11px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Prioritized actions</div>
                  <div className="space-y-2">
                    {report.actions.map((a, i) => (
                      <div key={i} className="flex items-start gap-2.5 rounded-xl border border-navy-100 bg-white p-3 dark:border-slate-800 dark:bg-[#211c33]">
                        <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-accent-500 text-[10px] font-bold text-white">{i + 1}</span>
                        <div className="min-w-0">
                          <div className="text-[12px] font-semibold">{a.title}</div>
                          <div className="text-[11px] leading-relaxed text-navy-500 dark:text-slate-400">{a.detail}</div>
                        </div>
              ...
              <div className="mt-4 text-center text-[10px] text-navy-400 dark:text-slate-600">Generated {new Date(report.generated_at).toLocaleString()}</div>
            </>
          )}
        </div>
      </div>
