...
export default function DemandForecasting({ plantIds }: { plantIds?: string[] } = {}) {
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4">
        <h1 className="text-[20px] font-semibold tracking-tight">Demand Forecasting</h1>
        <p className="text-[14px] text-navy-500 dark:text-slate-500">ML-powered · 3M to 24M horizons · Confidence intervals</p>
      </div>

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
        {/* Material picker */}
        <div>
          <div className="mb-2 text-[12.5px] font-semibold uppercase tracking-wide text-navy-400 dark:text-slate-500">Select Material</div>
            ...
            {list.map((m) => (
              <button key={m.id} onClick={() => setSelId(m.id)}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="font-mono text-[11.5px] text-navy-400 dark:text-slate-500">{m.id} · {m.plant_id}</div>
                </div>
                <span className={`rounded px-1.5 py-0.5 text-[11.5px] font-bold ${XYZ_TONE[m.xyz]}`}>{m.xyz}</span>
                <span className="text-[12px] text-navy-500 dark:text-slate-400">{m.avg_monthly_demand}/mo</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <div className="font-mono text-[13.5px] text-navy-500 dark:text-slate-400">{material.id} — {material.description}</div>
              <div className="text-[12.5px] text-navy-400 dark:text-slate-500">{material.plant_id} · {material.category} · XYZ: {material.xyz}</div>
            </div>
            <div className="flex gap-1">
              {HORIZONS.map((h) => (
                <button key={h} onClick={() => setHorizon(h)}
                  className={`rounded-lg px-2.5 py-1 text-[13px] font-semibold ${horizon === h ? 'bg-accent-500 text-white' : 'bg-navy-100 text-navy-600 hover:bg-navy-200 dark:bg-slate-800 dark:text-slate-300'}`}>{h}M</button>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-navy-100 bg-white p-4 dark:border-slate-800 dark:bg-[#211c33]">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-[14px] font-semibold">Demand Forecast — {horizon}M Horizon</span>
              <span className="flex items-center gap-2 text-[12.5px] text-navy-500 dark:text-slate-400">Model Accuracy
          ...
          <div className="rounded-xl bg-accent-500/5 p-3.5 dark:bg-accent-500/10">
            <div className="mb-1 flex items-center gap-1.5 text-[13.5px] font-semibold text-accent-700 dark:text-accent-300"><Sparkles className="h-3.5 w-3.5" /> AI Forecast Insight</div>
            <p className="text-[14px] leading-relaxed text-navy-700 dark:text-slate-300">{fc.insight}</p>
          </div>
        </div>
      </div>
    </div>
...
function Stat({ value, label, sub, tone }: { value: string; label: string; sub: string; tone?: 'accent' }) {
  return (
    <div className="rounded-xl border border-navy-100 bg-white p-3.5 dark:border-slate-800 dark:bg-[#211c33]">
      <div className={`text-[21px] font-semibold ${tone === 'accent' ? 'text-accent-600 dark:text-accent-400' : ''}`}>{value}</div>
