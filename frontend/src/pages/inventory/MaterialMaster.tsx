...
export default function MaterialMaster({ plantIds }: { plantIds: string[] }) {
  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-4">
        <h1 className="flex items-center gap-2 text-[20px] font-semibold tracking-tight"><List className="h-5 w-5 text-accent-500" /> Material master</h1>
        <p className="text-[14px] text-navy-500 dark:text-slate-500">Every SKU with its live forecast &amp; policy — the same metrics as the Savings Wizard. Click any material for the full 360°.</p>
      </div>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <div className="flex flex-wrap gap-1 rounded-lg border border-navy-100 p-0.5 dark:border-slate-800">
          {FILTERS.map((f) => (
            <button key={f.k} onClick={() => setFilter(f.k)} className={`rounded-md px-2.5 py-1 text-[12.5px] font-medium ${filter === f.k ? 'bg-accent-500 text-white' : 'text-navy-600 hover:bg-navy-50 dark:text-slate-300 dark:hover:bg-white/5'}`}>{f.label}</button>
          ))}
        </div>
        <span className="text-[12.5px] text-navy-400 dark:text-slate-500">{mats.length} materials</span>
        <div className="ml-auto flex items-center gap-2 rounded-lg border border-navy-200 bg-white px-2.5 py-1 dark:border-slate-700 dark:bg-slate-900">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search material, category, supplier…" className="w-56 border-none bg-transparent text-[13.5px] outline-none placeholder:text-navy-400 dark:text-slate-200" />
        </div>
      </div>
